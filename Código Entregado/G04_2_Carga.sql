...
-- Funcion que se encarga de insertar una billetera a los usuarios cuando se agrega una moneda
create or replace function trfn_g04_insertBilleteraNueva() returns trigger as $$
    declare
        tupla record;
    begin
        for tupla in select id_usuario from g04_usuario loop
            insert into g04_billetera values (tupla.id_usuario, new.moneda, 0);
        end loop;
        return new;
    end $$ language 'plpgsql';

-- Trigger para el insert de una nueva moneda actualice las billeteras de los usuarios
...
-- Funcion que ejecuta el tr que inserta todas las billeteras disponibles de la tabla moneda, por cada usuario nuevo
create or replace function trfn_g04_insertUsuarios() returns trigger as $$
    declare
        tupla record;
    begin
        for tupla in select moneda from g04_moneda loop
            insert into g04_billetera values (new.id_usuario, tupla.moneda, 0);
        end loop;
        return new;
    end $$ language 'plpgsql';

-- Trigger que inicializa todas las billeteras para un nuevo usuario
...
-- Funcion que se encarga de retornar el max(id) + 1 de la tabla g04_orden
...
-- Funcion que se encarga de retornar el max(fecha) de un movimiento + 1 segundo
...
-- Procedimiento que inserta x cantidad ordenes segun un mercado
create or replace procedure pr_g04_insertOrdenes(mercadoVar varchar(20),moneda_oVar varchar(10) ,moneda_dVar varchar(10),cantOrdenes integer) as $$
    declare
        i integer:= 1;
...
-- Funcion que inserta una cantidad de ordenes pasada por parametro para cada mercado
create or replace procedure pr_g04_insertaOrdenesMasivas(cantVar integer) as $$
    ...
    begin
        ...
        for tupla in (select m.nombre,m.moneda_d,m.moneda_o from g04_mercado m order by m.nombre) loop
            call pr_g04_insertOrdenes(tupla.nombre,tupla.moneda_o,tupla.moneda_d,cantVar);
...
-- Crypto vs Estables 60 filas = 5 (monedas estables) * 12 (monedas crypto no estables)
...
call pr_g04_insertUsuarios(100);

-- Inserts en la tabla G04_Ordenes
call pr_g04_insertaOrdenesMasivas(100);
