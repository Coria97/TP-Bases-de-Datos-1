    ...
    begin
        loop
            exit when i > cantOrdenes;
                ...
                if (i%2 = 0) then
                    if(not exists(select 1 from g04_billetera b where (b.id_usuario = i) and (b.moneda = moneda_oVar)))then
                        insert into g04_billetera values (i, moneda_oVar, randomVar);
                    end if;
                    insert into g04_orden values (idVar + i, mercadoVar, i ,'compra', current_date, NULL, random() * 10, random() * randomVar, 'activo');
                else
                    if(not exists(select 1 from g04_billetera b where (b.id_usuario = i) and (b.moneda = moneda_dVar)))then
                        insert into g04_billetera values (i, moneda_dVar, randomVar);
                    end if;
...
call pr_g04_insertaOrdenesMasivas(10);