...
create or replace procedure pr_g04_insertOrdenes(mercadoVar varchar(20),moneda_oVar varchar(10) ,moneda_dVar varchar(10),idVar bigint,cantOrdenes integer) as $$
    --Nosotros asumimos que el usuario invierte todo el capital en las ordenes de compra
    declare
        i integer;
    ...
    begin
        ...
        loop
            exit when i > cantOrdenes;
                ...
                if (i%2 = 0) then
                    if(not exists(select 1 from g04_billetera where moneda = moneda_oVar))then
                        insert into g04_billetera values (i,moneda_oVar,0);
                    end if;
                    insert into g04_orden values (idVar + i, mercadoVar, i ,'compra', current_date, NULL, random() * 10, random() * randomVar, 'activo');
                    raise warning 'usuario: % mercado : % randomVar : % ' ,i,moneda_oVar,randomVar;
                else
                    if(not exists(select 1 from g04_billetera where moneda = moneda_dVar))then
                        insert into g04_billetera values (i,moneda_dVar,0);
                    end if;
                    insert into g04_orden values (idVar + i, mercadoVar, i ,'venta', current_date, NULL, random() * 10, random() * randomVar, 'activo');
                    raise warning 'usuario: % mercado : % randomVar : % ' ,i,moneda_dVar,randomVar;
                end if;
                i:= i+1;
        end loop;
    end $$ language 'plpgsql';
...
create or replace procedure pr_g04_insertaOrdenesMasivas(cantVar integer) as $$
    ...
    begin
        idOrden:= 1;
        for tupla in (select m.nombre,m.moneda_d,m.moneda_o from g04_mercado m order by m.nombre asc) loop
            raise warning 'pr usuario: % mercado : % randomVar : % ' ,tupla.nombre,tupla.moneda_d,tupla.moneda_o;
            call pr_g04_insertOrdenes(tupla.nombre,tupla.moneda_d,tupla.moneda_o,idOrden,cantVar);
            idOrden:= idOrden + cantVar;
        end loop;
    end $$ language 'plpgsql';

call pr_g04_insertaOrdenesMasivas(2);
